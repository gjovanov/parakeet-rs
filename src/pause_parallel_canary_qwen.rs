//! Pause-based parallel transcription using multiple CanaryQwen model instances
//!
...
/// Configuration for pause-based parallel CanaryQwen transcription
#[derive(Debug, Clone)]
pub struct PauseParallelCanaryQwenConfig {
...
impl Default for PauseParallelCanaryQwenConfig {
    fn default() -> Self {
        Self {
            num_threads: 4,
...
impl PauseParallelCanaryQwenConfig {
        ...
        while let Some(result) = pending.remove(&self.next_emit_idx) {
            ...
            if !text.is_empty() {
                ...
                if result.start_time >= self.last_emitted_end_time - 0.1 {
                    ...
                    eprintln!(
                        "[PauseParallelCanaryQwen] Emitting segment {} [{:.1}s-{:.1}s] (inference: {}ms)",
    ...
    for i in 1..words.len() {
        if words[i].to_lowercase() == words[i - 1].to_lowercase() && words[i].len() > 1 {
            ...
            if consecutive_count >= 3 {
                ...
                if truncate_at > 0 {
                    eprintln!(
                        "[PauseParallelCanaryQwen] Truncating hallucination at word {}: '{}'",
                        truncate_at, words[i]
                    );
        ...
        for i in 0..=(words.len() - pattern_len * 3) {
            ...
            while j + pattern_len <= words.len() {
                    ...
                    .all(|(a, b)| a.to_lowercase() == b.to_lowercase())
                {
                    ...
                    if pattern_count >= 3 {
                        eprintln!(
                            "[PauseParallelCanaryQwen] Truncating repeated phrase at {}: '{}'",
                            i,
                            pattern.join(" ")
                        );
                        if i > 0 {
...
/// Pause-based parallel CanaryQwen transcriber
pub struct PauseParallelCanaryQwen {
    /// Configuration
    config: PauseParallelCanaryQwenConfig,
...
impl PauseParallelCanaryQwen {
    /// Create a new pause-based parallel CanaryQwen transcriber
    pub fn new(
        model_dir: &Path,
        exec_config: Option<ModelConfig>,
        config: Option<PauseParallelCanaryQwenConfig>,
    ) -> Result<Self> {
        let config = config.unwrap_or_default();

        eprintln!(
            "[PauseParallelCanaryQwen] Initializing with {} threads, language: {}, pause: {}ms",
        ...
        eprintln!("[PauseParallelCanaryQwen] Loading {} model instances...", config.num_threads);
        ...
        for i in 0..config.num_threads {
            ...
            let handle = thread::spawn(move || {
                eprintln!("[PauseParallelCanaryQwen] Worker {} loading model...", i);
                ...
                let mut model = match CanaryQwenModel::from_pretrained(&model_dir, Some(exec_config), Some(canary_qwen_config)) {
                    Ok(m) => m,
                    Err(e) => {
                        eprintln!("[PauseParallelCanaryQwen] Worker {} failed to load model: {}", i, e);
                        return;
                    }
                };
                eprintln!("[PauseParallelCanaryQwen] Worker {} ready (language: {})", i, language);
                    ...
                    match job {
                        Ok(job) => {
                            ...
                            let text = match model.transcribe(&job.audio) {
                                ...
                                Err(e) => {
                                    eprintln!(
                                        "[PauseParallelCanaryQwen] Worker {} inference error: {}",
                                        i, e
                                    );
                ...
                eprintln!("[PauseParallelCanaryQwen] Worker {} shutting down", i);
            });
        ...
        eprintln!(
            "[PauseParallelCanaryQwen] All {} workers initialized in {:.2}s",
            config.num_threads,
    ...
    /// Create a new pause-based parallel CanaryQwen transcriber with optional diarization
    #[cfg(feature = "sortformer")]
    pub fn new_with_diarization<P1: AsRef<Path>, P2: AsRef<Path>>(
        ...
        if let Some(diar_path) = diar_model_path {
            eprintln!("[PauseParallelCanaryQwen] Creating diarizer from {:?}", diar_path.as_ref());
    ...
    fn dispatch_segment(&mut self, segment_audio: Vec<f32>, start_time: f32, end_time: f32) {
        ...
        eprintln!(
            "[PauseParallelCanaryQwen] Dispatching segment {} [{:.1}s-{:.1}s] ({:.1}s audio + {:.1}s context)",
        ...
        if let Err(e) = self.job_tx.send(job) {
            eprintln!("[PauseParallelCanaryQwen] Failed to dispatch job: {}", e);
...
impl StreamingTranscriber for PauseParallelCanaryQwen {
    fn model_info(&self) -> ModelInfo {
        ModelInfo {
            id: "pause-parallel-canary-qwen".to_string(),
            display_name: format!("Pause-Parallel CanaryQwen ({}T)", self.config.num_threads),
            description: format!(
    ...
    fn push_audio(&mut self, samples: &[f32]) -> Result<StreamingChunkResult> {
        ...
        if current_segment_duration > self.config.max_segment_duration_secs {
            eprintln!(
                "[PauseParallelCanaryQwen] Forcing segment break at {:.1}s (max duration reached)",
                chunk_end_time
            );
            let segment_audio = std::mem::take(&mut self.current_segment_audio);
    ...
    fn finalize(&mut self) -> Result<StreamingChunkResult> {
        eprintln!("[PauseParallelCanaryQwen] Finalizing...");
        ...
        eprintln!("[PauseParallelCanaryQwen] Finalized with {} segments", segments.len());
...
impl Drop for PauseParallelCanaryQwen {
    fn drop(&mut self) {
        eprintln!("[PauseParallelCanaryQwen] Shutting down {} workers...", self.workers.len());
        self.shutdown.store(true, Ordering::Relaxed);

        for (i, worker) in self.workers.drain(..).enumerate() {
            if worker.join().is_err() {
                eprintln!("[PauseParallelCanaryQwen] Worker {} panicked", i);
            }
        }

        eprintln!("[PauseParallelCanaryQwen] Shutdown complete");
    }
}
...
mod tests {
    ...
    fn test_config_default() {
        let config = PauseParallelCanaryQwenConfig::default();
    ...
    #[test]
    fn test_config_new() {
        let config = PauseParallelCanaryQwenConfig::new(2, "de".to_string());
    ...
    #[test]
    fn test_truncate_hallucination_no_hallucination() {
        let text = "this is a normal sentence with no repeated words at all";
        let result = truncate_hallucination(text);
        assert_eq!(result, text);
    }

    #[test]
    fn test_ordered_merger_in_order() {
        let mut merger = OrderedMerger::new();
        let mut pending = BTreeMap::new();

        pending.insert(0, TranscriptionResult {
            segment_idx: 0,
        ...
        pending.insert(1, TranscriptionResult {
            segment_idx: 1,
        ...
        let segments = merger.process(&mut pending);
        assert_eq!(segments.len(), 2);
        assert_eq!(segments[0].text, "hello world");
        assert_eq!(segments[1].text, "foo bar");
        assert_eq!(merger.next_emit_idx, 2);
    }

    #[test]
    fn test_ordered_merger_gap() {
        let mut merger = OrderedMerger::new();
        let mut pending = BTreeMap::new();

        // Insert segment 1 but not 0 - should emit nothing
        pending.insert(1, TranscriptionResult {
            segment_idx: 1,
        ...
        assert_eq!(merger.next_emit_idx, 0);
    ...
    fn test_samples_per_second() {
        let config = PauseParallelCanaryQwenConfig::default();
        assert_eq!(config.samples_per_second(), 16000);
    }
}
