...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Testing Silero VAD with normalized audio...\n");
    ...
    println!("Audio: {} samples, max amplitude: {:.4}", samples.len(), max_val);

    // Normalize audio to [-1, 1] range
    let normalized: Vec<f32> = samples.iter().map(|&s| s / max_val).collect();

    // Test VAD with normalized audio
    println!("\n=== Raw VAD with normalized audio ===");
    ...
    for (i, chunk) in normalized.chunks(chunk_size).enumerate() {
        let prob = vad.process(chunk)?;
        total_chunks += 1;

        if prob > 0.5 {
            speech_chunks += 1;
            if speech_chunks <= 20 || i % 50 == 0 {
                println!("Chunk {:3} ({:5.2}s): prob = {:.4}", i, i as f32 * 0.032, prob);
            }
        }
    }
    ...
    // Test VadSegmenter with normalized audio
    println!("\n=== VadSegmenter with normalized audio ===");
    ...
    for (i, chunk) in normalized.chunks(chunk_duration_samples).enumerate() {
