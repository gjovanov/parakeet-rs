//! Streaming wrapper for Whisper models
//!
//! This module provides a streaming interface for Whisper encoder-decoder models,
//! similar to RealtimeCanary but adapted for Whisper's 30-second context window.
//!
//! ## Text Confirmation Strategy
//!
//! Since Whisper re-transcribes a sliding window of audio, we need to handle
//! overlapping/repeated text carefully:
//!
//! 1. Each transcription covers the current buffer window (up to 30s)
...
pub struct RealtimeWhisperConfig {
    /// Buffer size in seconds before processing (max 30s for Whisper)
    ...
    pub task: String,
    /// Number of words that must match to confirm text
    pub confirmation_words: usize,
}
...
impl RealtimeWhisperConfig {
    /// Fast response mode (lower latency, may have more corrections)
    pub fn speedy() -> Self {
        Self {
    ...
    /// High quality mode (more context, higher latency)
    pub fn quality() -> Self {
        Self {
...
impl RealtimeWhisper {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        let model = WhisperModel::from_pretrained(model_path, exec_config, Some(whisper_config))?;

        // Clamp buffer to 30s max (Whisper's context window)
        let buffer_size_secs = config.buffer_size_secs.min(30.0);
    ...
    pub fn push_audio_internal(&mut self, samples: &[f32]) -> Result<WhisperChunkResult> {
        // Add to buffer
    ...
    fn process_buffer(&mut self) -> Result<WhisperChunkResult> {
        ...
        // Split into words for comparison (use owned strings to avoid borrow issues)
        let current_words: Vec<String> = text.split_whitespace().map(String::from).collect();
        let last_words: Vec<String> = self.last_transcription.split_whitespace().map(String::from).collect();
        ...
        let stable_words: Vec<String> = current_words[..common_prefix_len].to_vec();

        // New/unstable words at the end
        let unstable_words: Vec<String> = current_words[common_prefix_len..].to_vec();

        // Check if we should confirm more words
        let words_to_confirm = if common_prefix_len > self.confirmed_word_count {
            let new_confirmed: Vec<String> = stable_words[self.confirmed_word_count..].to_vec();
        ...
        if let Some(new_confirmed_text) = words_to_confirm {
            ...
            eprintln!("[RealtimeWhisper] Confirmed: \"{}\" (total: {} words)",
        ...
        // Return the unstable portion as partial
    ...
    pub fn finalize_internal(&mut self) -> Result<WhisperChunkResult> {
    ...
    ///
    /// When the token is set to false, the model will stop decoding
    /// at the next opportunity, allowing fast shutdown even during
    /// long-running inference operations.
    pub fn set_cancellation_token(&mut self, token: Arc<AtomicBool>) {
        self.model.set_cancellation_token(token);
    }
}
...
impl StreamingTranscriber for RealtimeWhisper {
    fn model_info(&self) -> ModelInfo {
        ...
        ModelInfo {
            id: variant.as_str().to_string(),
            display_name: match variant {
                WhisperVariant::LargeV2 => "Whisper Large v2".to_string(),
                WhisperVariant::LargeV3 => "Whisper Large v3".to_string(),
            },
            description: format!("OpenAI's {} for multilingual ASR and translation", variant.as_str()),
    ...
    fn push_audio(&mut self, samples: &[f32]) -> Result<StreamingChunkResult> {
        ...
        if !result.text.is_empty() {
            segments.push(TranscriptionSegment {
                ...
                end_time: self.total_duration_secs(),
                speaker: None,
                confidence: None,
                is_final: false,
            });
        ...
        if !segments.is_empty() {
            eprintln!("[RealtimeWhisper] Returning {} segment(s) ({} final, {} partial)",
                segments.len(),
        ...
        Ok(StreamingChunkResult {
            ...
            total_duration: self.total_duration_secs(),
        })
    }
...
mod tests {
    ...
    fn test_config_modes() {
        let speedy = RealtimeWhisperConfig::speedy();
        assert_eq!(speedy.buffer_size_secs, 10.0);
        assert_eq!(speedy.confirmation_words, 3);

        let quality = RealtimeWhisperConfig::quality();
        assert_eq!(quality.buffer_size_secs, 25.0);
        assert_eq!(quality.confirmation_words, 8);
    }
}
