...
struct Args {
    ...
    /// Enable FAB live transcription forwarding (default: true when FAB_URL is set)
    #[arg(long, env = "FAB_ENABLED")]
    fab_enabled: Option<bool>,

    /// FAB send type: "growing" (cumulative text) or "confirmed" (finalized segment text)
    #[arg(long, env = "FAB_SEND_TYPE", default_value = "growing")]
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let fab_enabled = args.fab_enabled.unwrap_or(args.fab_url.is_some());
    let fab_send_type = match args.fab_send_type.as_str() {
        "growing" | "confirmed" => args.fab_send_type.clone(),
        other => {
            eprintln!("[FAB] Warning: unknown FAB_SEND_TYPE '{}', defaulting to 'growing'", other);
            "growing".to_string()
        }
    };
    let fab_client = Some(
    ...
    let fab_url = args.fab_url.clone();
    if fab_enabled {
        if let Some(ref url) = fab_url {
            eprintln!("[FAB] Live transcription forwarding enabled: {} (send_type: {})", url, fab_send_type);
        } else {
            eprintln!("[FAB] Live transcription forwarding enabled (no default URL, per-session only, send_type: {})", fab_send_type);
        }
    } else {
        eprintln!("[FAB] Live transcription forwarding: disabled");
    }
