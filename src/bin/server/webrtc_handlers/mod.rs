...
pub async fn handle_socket(socket: WebSocket, session_id: String, state: Arc<AppState>) {
    ...
    peer_connection.on_peer_connection_state_change(Box::new(move |state| {
        ...
        if matches!(state, webrtc::peer_connection::peer_connection_state::RTCPeerConnectionState::Failed
            | webrtc::peer_connection::peer_connection_state::RTCPeerConnectionState::Disconnected) {
            eprintln!(
                "[WebRTC] Client {} peer connection lost, client reconnect expected",
                &client_id_clone[..8]
            );
        }
        Box::pin(async {})
